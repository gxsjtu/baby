...
Page({
        ...
        if (option == 'census') {
            ...
            this.setData({
                ...
                note: [{
                    ...
                    items: [
                        '1、本市户籍幼儿',
                        '2、持有上海居住证并且积分达到120分的人员子女。',
                        '3、引进人才子女B类（CR、FR等开头），外籍人员'
                    ]
                },
                {
                    title: '收费标准：',
            ...
            this.setData({
                ...
                note: [{
                    title: '参加对象:',
                    items: [
                        '1、中小学校（含中专、职校、技校、特殊学校）在册学生和入托入园在册儿童。',
                        '2、本市户籍的18周岁以下未入学散居少儿（包括《上海市居住证》达标准积分人员同住子女）。'
                    ]
                },
                {
                    title: '收费标准：',
                    items: [
                        '2016学年（2016.9-2017.8）每人80元。'
                    ]
                },
                {
                    title: '缴费方式：',
                    items: [
                        '1、在校（园）的学生，向所在学校、幼儿园专门设立的少儿住院互助基金缴费窗口直接缴纳参保费；',
                        '2、其他散居少儿，到户籍（《上海市居住证》）所在地的社区卫生服务中心缴纳参保费。'
                    ]
                },
                {
                    title: '保障年度',
                    items: [
                        '1、以学年度计算，即当年的9月1日至次年的8月31日。',
                        '2、新生儿自出生之日起享受待遇；其他中途参加的少儿自交费之日起享受待遇。'
                    ]
                }]
            });
        }
    ...
    setOptInData: function (e) {
        ...
        actionSvc.goodsCount('', this.data.pageName).then((data) => {
            this.setData({
                'optIn.num': (data.data.data ? data.data.data : 0)
            });
            let goods = getApp().globalData.user.goods;
            let good = _.find(goods, { hospitalId: '', pageId: this.data.pageName });
            if (good) {
                //已点过赞
                this.setData({
    ...
    usefulClick: function (e) {
        ...
        if (this.data.optIn.enable[0]) {
            var actionSvc = new ActionSvc();
            actionSvc.clickGood('', this.data.pageName).then((data) => {
                if (data.data.message == 'OK') {
                    ...
                    //更新globalData
                    let goods = getApp().globalData.user.goods;
                    let good = {hospitalId: '', pageId: this.data.pageName};
                    goods.push(good);
                }
            });
        }
